...
1. The system collects all the files under the `submissionFiles` section. If any of those files are missing, the autograder aborts and reports those files are missing in the Grading Summary.
2. The `cmd` given in `pawtograder.yml` is run. This command should be configured to compile and run whatever tests you need. If this command fails, the failure is reported and no autograder results are given.
3. A grading summary is generated from the results of the tests run. This is what the students see on Pawtograder for a submission.
4. Any additional artifacts requested in the `pawtograder.yml` are generated and added to the grading summary as needed.
5. The environment is cleaned up for the next submission.
6. The `submissionFiles` are listed in the "Files" section of the student's results on Pawtograder.
...
In this section, you declare the files that are accepted by Pawtograder for manual grading and testing. These are split into two necessary sub-fields: _files_ and _testFiles_.

- _files_ are the source code and any auxiliary files you want to either automatically grade or manually grade.
...
Here is an example `pawtograder.yml` for a Python project. In this vesion, style was relegated to a separate test in the `gradedParts` property. Therefore, the linter was set to `ignore` since students always needed to see the autograder results.
